...
try {
    ...
    // Fetch settings
    $setStmt = $pdo->query("SELECT currencySymbol FROM SettingGeneral LIMIT 1");
    $settings = $setStmt ? $setStmt->fetch(PDO::FETCH_ASSOC) : [];
    $symbol = $settings['currencySymbol'] ?? '৳';

    // Fetch Products with filter logic
    $query = "SELECT p.*, c.name as categoryName FROM Product p LEFT JOIN Category c ON p.categoryId = c.id WHERE 1=1";
    $params = [];
    ...
    if ($sort === 'price_low') {
        $query .= " ORDER BY p.basePrice ASC";
    } elseif ($sort === 'price_high') {
        $query .= " ORDER BY p.basePrice DESC";
    } else {
...
$selectedCatDesc = 'Browse our complete collection of premium products.';
if (!empty($_GET['category'])) {
    $cStmt = $pdo->prepare("SELECT name FROM Category WHERE slug = ?");
    $cStmt->execute([$_GET['category']]);
    $cData = $cStmt->fetch(PDO::FETCH_ASSOC);
    if ($cData) {
        $selectedCatName = $cData['name'];
        $selectedCatDesc = 'Explore our curated selection of ' . htmlspecialchars($cData['name']) . '.';
    } else {
        $selectedCatName = 'Category Not Found';
    }
}
    ...
    <div class="max-w-[1200px] mx-auto px-6">
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="w-full lg:w-72 shrink-0">
                ...
                <form method="GET" action="shop" id="filter-sidebar" class="hidden lg:block bg-white rounded-3xl p-6 shadow-sm border border-slate-200 sticky top-28 space-y-8">
                    <div>
                        <h3 class="font-bold text-slate-800 mb-4 uppercase tracking-widest text-xs flex items-center gap-2"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg> Search</h3>
                        <div class="relative">
                    ...
                    <div>
                        ...
                        <select name="sort" onchange="this.form.submit()" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-[#003e86]/20 focus:border-[#003e86] outline-none transition-all cursor-pointer">
                            <option value="newest" <?php echo ($_GET['sort'] ?? '') === 'newest' ? 'selected' : ''; ?>>Newest Arrivals</option>
            ...
            <div class="flex-1">
                ...
                <div class="mb-8 bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="relative z-10">
                        <h1 class="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tight mb-2"><?php echo htmlspecialchars($selectedCatName); ?></h1>
                        <p class="text-sm sm:text-base text-slate-500 font-medium"><?php echo htmlspecialchars($selectedCatDesc); ?></p>
                    </div>
                    <div class="relative z-10 bg-slate-50 px-5 py-3.5 rounded-2xl border border-slate-100 flex items-center gap-4 shadow-inner shrink-0">
                        ...
                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-tight">Items<br>Found</span>
                    </div>
                    <!-- Decorative blur -->
                    ...
                    <?php if(empty($products)): ?>
                        <div class="col-span-full py-16 text-center bg-white rounded-3xl border border-slate-200 shadow-sm">
                            <div class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
                                <svg class="w-10 h-10 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" /></svg>
                            </div>
                            <h3 class="text-xl font-bold text-slate-800 mb-2">No products found</h3>
                            <p class="text-slate-500 mb-6 font-medium">Try adjusting your filters or search term to find what you're looking for.</p>
                            <a href="shop" class="inline-flex items-center gap-2 bg-[#003e86] hover:bg-blue-800 text-white font-bold px-6 py-3 rounded-xl shadow-md transition-all">Clear all filters</a>
                        </div>
                    <?php else: ?>
